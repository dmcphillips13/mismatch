...
- The test dataset contains 51 synthetic queries generated by RAGAS `TestsetGenerator` using a mix of single-hop specific (50%), multi-hop abstract (25%), and multi-hop specific (25%) strategies across a sample of 100 documents (took too long to use all of the documents). The baseline uses dense cosine retrieval from Qdrant (top 6 documents) with gpt-4o-mini for generation.
...
- Faithfulness is the strongest metric (0.88), which means the LLM is mostly grounding its answers in the retrieved context rather than hallucinating. This makes sense given that the pipeline uses math for the odds tables and only asks the LLM to generate rationale text from the provided docs.
